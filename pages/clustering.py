...
class AnalysisClusteringPage(tk.Frame):
    ...
    def init_frame(self):
        ...
        self.df = self.controller.DF
        # add control pane
        ctrl_pane = ClusteringControlPane(vertical_split, self)
...
class ClusteringControlPane(tk.Frame):
    ...
    def init_frame(self):
        ...
        row += 1
        label_method = tk.Label(self, text="clustering method",
                                font=LABEL, bg='#F3F3F3')
        label_method.grid(row=row, column=0, columnspan=3)
        row += 1
        method = tk.StringVar(self)
        methods = ['kmeans', 'hierarchy']
        method.set('kmeans')
        chose_method = tk.OptionMenu(self, method, *methods)
        chose_method.grid(row=row, column=0, columnspan=3)
        self.method = method
