...
class RegressionResultPane(tk.Frame):
    ...
    def init_frame(self):
        # coefficients
        self.coef_label = tk.Label(self, text="Model Coefficients", font=SECTION)
        self.coef_label.pack()
        self.coef_table = Table(self, self.coef_df, width=500)
        self.coef_table.pack(fill=tk.X, expand=1)

        # model statistics
        self.stat_label = tk.Label(self, text="Model Statistics", font=SECTION)
        self.stat_label.pack()
        self.stat_table = Table(self, self.stat_df, width=500)
        self.stat_table.pack(fill=tk.X, expand=1)
